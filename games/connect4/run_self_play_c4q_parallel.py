...
from games.connect4.modules import ConvNetConnect4Advantage
...
def run_training():
    ...
    network = ConvNetConnect4Advantage()
    network.share_memory()

    policy_gen = EpsilonGreedy
    policy_args = []
    policy_kwargs = dict(memory_size=20000, epsilon=0.1, env_gen=Connect4Env)
    self_play = True
    if self_play:
        opposing_policy_gen = EpsilonGreedy
        opposing_policy_args = []
        opposing_policy_kwargs = dict(memory_size=20000, epsilon=0.1, env_gen=Connect4Env)
        evaluation_policy_gen = Random
        evaluation_policy_args = []
        evaluation_policy_kwargs = dict(env_gen=Connect4Env, player=-1)

        # evaluation_policy_args = []
        # evaluation_policy_kwargs = dict(memory_size=50000, env_gen=Connect4Env, player=-1)
